...
export async function syncCollection(accountId: string, collection: SyncCollection, value: unknown): Promise<void> {
  try {
    if (supabase) await supabase.from("flixcasa_sync").upsert({ account_id: accountId, collection, data: value, updated_at: new Date().toISOString() }, { onConflict: "account_id,collection" });
  } catch {
...
export async function getSyncedCollections(accountId: string): Promise<Partial<Record<SyncCollection, unknown>>> {
  try {
    if (supabase) {
      const { data: remoteRows } = await supabase.from("flixcasa_sync").select("collection,data").eq("account_id", accountId);
...
export async function updatePresence(accountId: string, online: boolean): Promise<void> {
  try {
    if (supabase) await supabase.from("profiles").upsert({ id: accountId, is_online: online, last_seen: new Date().toISOString() }, { onConflict: "id" });
  } catch {
